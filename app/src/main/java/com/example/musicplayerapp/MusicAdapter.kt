...
import com.example.musicplayerapp.databinding.MoreFeaturesBinding
...
class  MusicAdapter(private val context: Context, private var musicList: ArrayList<Music>, private val playlistDetails: Boolean = false,
    ...
    override fun onBindViewHolder(holder: MyHolder, position: Int) {
        ...
        Glide.with(context)
            ...
            // if image cant be loaded then this img will be displayed
        ...
        if(playlistDetails ) {
            holder.moreDots.setOnClickListener {
                ...
                dialog.setContentView(R.layout.more_dots_bottom_sheet_dialog)
                dialog.show()
                dialog.findViewById<LinearLayout>(R.id.removeSong)?.setOnClickListener {
                    Log.d("remove", "remove")
                    // Remove the song from the playlist
                    PlaylistActivity.musicPlaylist.ref[PlaylistDetails.currentPlaylistPos].playlist.removeAt(
                        position
                    )
                    // Refresh the adapter after removing the song
                    refreshPlaylist()
                    dialog.dismiss()
                }
                dialog.findViewById<LinearLayout>(R.id.songInfo)?.setOnClickListener {
                    dialog.dismiss()
                    Log.d("info","info")
                    // Inflate the custom details view
...
//                        .setBackground(ColorDrawable(0x99000000.toInt()))
                ...
                dialog.findViewById<LinearLayout>(R.id.addToPlayNext)?.setOnClickListener {
                    try {
...
//                        .setBackground(ColorDrawable(0x99000000.toInt()))
            ...
            holder.moreDots.setOnClickListener {
                val dialog = BottomSheetDialog(context)
                dialog.setContentView(R.layout.more_dots_main_bottom_sheet_dialog)
                dialog.show()
                dialog.findViewById<LinearLayout>(R.id.songInfo)?.setOnClickListener {
                    dialog.dismiss()
                    Log.d("info","info")
...
//                        .setBackground(ColorDrawable(0x99000000.toInt()))
            ...
            selectionActivity ->{
                ...
                holder.root.setOnClickListener {
                    when{
                        ...
                        // if we are clicking again on the now playing song (means id are equal) then we will not change the position
    ...
    private fun addSong(song: Music): Boolean{
        PlaylistActivity.musicPlaylist.ref[PlaylistDetails.currentPlaylistPos].playlist.forEachIndexed { index, music ->
            if(song.id == music.id){
                // if song is already added in the playlist so remove that song from the search view all songs
