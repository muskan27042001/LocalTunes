...
// we can not set image in notification using Glide so we use this
...
fun setSongPosition(increment: Boolean){
    // if repeat is not true then only position will increment / decrement
    if(!PlayerActivity.repeat){
        if(increment)
        {
            // if we are on last song then set it to first song
            if(PlayerActivity.musicListPA.size - 1 == PlayerActivity.songPosition)
                PlayerActivity.songPosition = 0
            // else increase song position
            else ++PlayerActivity.songPosition
        }
        // decrement
        else
        {
            // if we are on first song then set the last song
            if(0 == PlayerActivity.songPosition)
                PlayerActivity.songPosition = PlayerActivity.musicListPA.size-1
            // else decrease song position
            else --PlayerActivity.songPosition
        }
    }
}
