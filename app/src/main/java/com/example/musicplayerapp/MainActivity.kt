...
class  MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
      ...
      //  setTheme(currentThemeNav[themeIndex])
        ...
        // Navigation Drawer
        // appears when the user swipes from the edge of the screen or taps on the app icon in the action bar.
        ...
        if(requestRuntimePermission()){
            Log.d("Log 1","")
            // if permission is granted then initialize layout
            initializeLayout()
        ...
        // Navigation drawer items selection
...
//                R.id.navFeedback -> startActivity(Intent(this@MainActivity, FeedbackActivity::class.java))
    ...
    private val requestPermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
            if (isGranted) {
                Log.d("Log 5","")
                // Permission granted, initialize layout or perform required actions
                Log.d("Permission", "Granted")
                Log.d("if requestRuntimePermission","if")
                // if permission is granted then initialize layout
                initializeLayout()
                //for retrieving favourites data using shared preferences
                FavouriteActivity.favouriteSongs = ArrayList()
                ...
                // Optionally, show rationale and request again
                Toast.makeText(
                    this,
                    "Permission denied. Please grant storage permission to use the app.",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
    ...
    /*private fun requestRuntimePermission() :Boolean{
        Log.d("requesRuntimePermission","func")

        if(Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU){
            Log.d("requesRuntimePermission","func1")
            if(ActivityCompat.checkSelfPermission(this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE)
                != PackageManager.PERMISSION_GRANTED) {
                Log.d("requesRuntimePermission","func1.1")
    ...
    /*override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if(requestCode == 13){
            Log.d("requesRuntimePermission","result")
            if(grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                Log.d("requesRuntimePermission","result1")
                Toast.makeText(this, "Permission Granted",Toast.LENGTH_SHORT).show()
                // if permission is granted then initialize layout
                initializeLayout()
            }
            else // otherwise request permission again
                ActivityCompat.requestPermissions(this, arrayOf(android.Manifest.permission.WRITE_EXTERNAL_STORAGE), 13)
        }
    }*/
    ...
    private fun initializeLayout(){
        ...
        // getting all audio data
        MusicListMA = getAllAudio()
        // setting size so that only 13 audio are available not all(it saves memory)
        binding.musicRV.setHasFixedSize(true)
        binding.musicRV.setItemViewCacheSize(13)
        // setting layout of adapter
        binding.musicRV.layoutManager = LinearLayoutManager(this@MainActivity)
        musicAdapter = MusicAdapter(this@MainActivity, MusicListMA)
        binding.musicRV.adapter = musicAdapter

        binding.totalSongs.text  = "Songs ( "+musicAdapter.itemCount + " )"
    ...
    private fun getAllAudio(): ArrayList<Music>{
        // initializing empty Arraylist of type Music
        val tempList = ArrayList<Music>()
        // getting only audio files
        val selection = MediaStore.Audio.Media.IS_MUSIC +  " != 0"
        // things we want to retrieve
        val projection = arrayOf(MediaStore.Audio.Media._ID,MediaStore.Audio.Media.TITLE,MediaStore.Audio.Media.ALBUM,
            MediaStore.Audio.Media.ARTIST,MediaStore.Audio.Media.DURATION,MediaStore.Audio.Media.DATE_ADDED,
            MediaStore.Audio.Media.DATA, MediaStore.Audio.Media.ALBUM_ID)
        // cursor gives the result of our query
